#!/usr/bin/env bash
#===============================================================================
# psiphon-multi-region / Psiphon Multi-Region Auto-Installer v5.1
...
LOOPBACK_SERVICE_FILE="/etc/systemd/system/psiphon-loopback-alias.service"
...
step() {
    ...
    echo -e "${C_BLUE}==> ${C_CYAN}$*${C_RESET}" | tee -a "$LOG_FILE"
}
...
on_error() {
    ...
    # SSH protection: if SSH is no longer listening, remove loopback alias
...
Environment variables (or use config.env):
  ...
  DRY  SSH_PORT             SSH port to protect              (default: 22)
  DRY_RUN}
...
#------------------------------------------------------------------------------
write_loopback_service() {
    if [[ "$DRY_RUN" == "1" ]]; then
        info "[DRY-RUN] would write ${LOOPBACK_SERVICE_FILE}"
        return 0
    fi

    cat > "$LOOPBACK_SERVICE_FILE" <<EOF
[Unit]
Description=Psiphon loopback alias ${LOOPBACK_ALIAS_IP}/32
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/bin/sh -c 'ip addr add ${LOOPBACK_ALIAS_IP}/32 dev ${LOOPBACK_ALIAS_DEV} 2>/dev/null || true'
ExecStop=/bin/sh -c 'ip addr del ${LOOPBACK_ALIAS_IP}/32 dev ${LOOPBACK_ALIAS_DEV} 2>/dev/null || true'
...
EOF
    ...
    systemctl enable --now psiphon-loopback-alias.service >/dev/null 2 psiphon-loopback-alias.service >/dev/null 2>&1 || true
_2_loopback() {
...
#------------------------------------------------------------------------------
# STEP 5: download binary + integrity verification
#------------------------------------------------------------------------------
step_5_binary() {
    ...
    if [[ -x "$dest" ]] && ("$dest" -version >/dev/null 2>&1 || "$dest" --help >/dev/null 2>&1); then
    ...
    printf '%s  %s\n' "$sum" "$dest" > "${BIN_DIR}/${BINARY_NAME}.sha256"
    info "SHA256: ${sum}"

    ( cd "$BIN_DIR" && sha256sum -c "${BINARY_NAME}.sha256" >/dev/null ) \
    ...
    if ! ("$dest" -version >/dev/null 2>&1 || "$dest" --help >/dev/null 2>&1); then
        warn "Binary did not answer -version/--help; continuing"
    fi
...
step_6_configs() {
    ...
    local i reg json iport socks_port content cur_region cur_socks cur_inbound
    ...
    for (( i=1; i<=PSIPHON_INSTANCES; i++ )); do
        ...
        if [[ -f "$json" && "$DRY_RUN" != "1" ]]; then
            cur_region="$(jq -r 'if .EgressRegion == null then "-" else .EgressRegion end' "$json" 2>/dev/null || echo "CHANGED")"
            cur_socks="$(jq -r '.LocalSocksProxyPort // empty' "$json" 2>/dev/null || echo "")"
            cur_inbound="$(jq -r '.TargetInboundPort // empty' "$json" 2>/dev/null || echo "")"

            if [[ "$cur_region" == "$reg" && "$cur_socks" == "$socks_port" && "$cur_inbound" == "$iport" ]]; then
                ok "Config ${i} unchanged (idempotent skip)"
                continue
            fi
        fi

        content="$(python3 - "$i" "$socks_port" "$iport" "$reg" "$SOCKS_LISTEN_IP" <<'PYEOF'
import json
import sys

i = int(sys.argv[1])
socks = int(sys.argv[2])
inbound = int(sys.argv[3])
reg = sys.argv[4]
listen = sys.argv[5]

egress = None if reg == "-" else reg
...
cfg = {
    ...
    "EgressRegion": egress,
    "EgressRegionCombo": egress,
    "ListenInterface": listen,
    "TargetInboundPort": inbound,
    "UpstreamProxyUrl": "",
...
PYEOF
    ...
    if [[ "$DRY_RUN" != "1" ]]; then
        chmod 644 "${CONF_DIR}"/*.json 2>/dev/null || true
    fi
...
WantedBy=multi-user.target
EOF
        ok "Unit written=multi-user.target
EOF
        ok "Unit written ${socks_port})"
    done
...
print_summary() {
    ...
    for (( i=1; i<=PSIPHON_INSTANCES; i++ )); do
        ...
        printf '%-8s | %-13s | %-11s | %-7s | %s\n' "$i" "$iport" "$socks_port" "$reg" "$status" | tee -a "$LOG_FILE"
    done
